...
export function FormField({ label, type = "input", value, onChange, options, placeholder, icon, hasError }) {
          ...
          >
            <div className="flex items-center gap-2">
              {options?.find((o) => o.label === value)?.icon && (
                <img
                  src={options.find((o) => o.label === value)?.icon}
                  alt={value}
                  width={24}
                  height={24}
          ...
          {open && (
            <ul className="absolute z-10 mt-2 w-full bg-gray-900 border border-gray-700 rounded-xl max-h-60 overflow-auto">
              {options.map((option) => (
                ...
                >
                  <img src={option.icon} alt={option.label} width={24} height={24} className="rounded-full" />
  ...
  className={`w-full bg-gray-800/50 backdrop-blur-sm rounded-xl px-4 py-4 text-white placeholder-gray-500 focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500/20 transition-all border ${hasError ? "border-red-500" : "border-gray-600/50"}`}
/>
