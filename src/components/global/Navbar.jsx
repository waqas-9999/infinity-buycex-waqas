...
export default function Navbar() {
  ...
  },
  {
    ...
    dropdown: [
      {
        name: "Project Funds Application",
        href: "#",
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      const clickedInside = Object.values(dropdownRefs.current).some((ref) => ref?.contains(e.target))
      if (!clickedInside) {
    ...
    >
      ...
      <div className="hidden md:flex items-center space-x-8 relative">
        {navItems.map((item, index) => (
          <div
            key={index}
            className={`relative ${index === navItems.length - 1 ? "dropdown-right" : ""}`}
            ...
            >
              ...
              <motion.span
                animate={{
                  ...
                  color: activeDropdown === item.label ? "#efb81c" : "currentColor",
                }}
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <div className="text-xs text-white/50 mt-1 group-hover:text-white/70">{link.description}</div>
                        </div>
                        <motion.div
